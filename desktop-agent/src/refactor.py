...
# Remove variables that will be passed as arguments
pipeline_body = re.sub(r'let mut read_stream = match stream.try_clone\(\) \{[^\}]+\};', '', pipeline_body, flags=re.DOTALL)
pipeline_body = re.sub(r'let _ = read_stream.set_read_timeout\(None\);', '', pipeline_body)
pipeline_body = re.sub(r'let mut write_tcp = match stream.try_clone\(\) \{[^\}]+\};', '', pipeline_body, flags=re.DOTALL)
pipeline_body = re.sub(r'let _ = write_tcp.set_write_timeout\(None\);', '', pipeline_body)
pipeline_body = re.sub(r'#\[cfg\(windows\)\]\s*\{[^\}]*let sock = stream\.as_raw_socket\(\);[^\}]+\}', '', pipeline_body, flags=re.DOTALL)
